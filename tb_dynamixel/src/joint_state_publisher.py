...
class JointStatePublisher():
    def __init__(self):
        ...
        self.srv = rospy.Service('kinect_controller/set_angle_calib', SetAngle, self.set_angle_srv)
        ...
        [rospy.Subscriber('/dynamixel/' + c + '/state', JointStateDynamixel, self.controller_state_handler) for c in self.controllers]
    ...
    def controller_state_handler(self, msg):
        ...
        # TODO move this awful hack to configuration
        if msg.name == 'kinect_joint':
            
            js.position = js.position + self.kinect_calib
            self.joint_states['kinect_joint'] = js
            return
        
        if msg.name == 'left_finger_joint':
        
            self.joint_states[msg.name] = js
            
            js.name = 'right_finger_joint'
            js.position = -1.0 * js.position
            self.joint_states['right_finger_joint'] = js
            return 
